    ...
    #            mesaj=fisier.read(1024)
        ...
        try:
            if linieDeStart[1]=='/api/utilizatori':          
                ...
                #print('dimensiune : '+str(len(payload.encode('utf-8'))))
                form_fields = payload.split('&')
                print('FIELDS : '+str(form_fields))

                # toate datele din formular vor fi stocate intr-un
                # dictionar, pentru serializare json
                user={}
                username=''
                parola=''
                ...
                for field in form_fields:
                    ...
                    if field.find('sexul')>-1:
                        ...
                        if sexul != "":
                            user['sexul']=sexul
                    if field.find('jucator')>-1:
                        jucator = field.split('=')[1]
                        if jucator != "":
                            user['jucator_preferat']=jucator
                    ...
                    if field.find('ora_nasterii')>-1:
                        ora_nasterii = field.split('=')[1]
                        if ora_nasterii != "":
                            user['ora_nasterii']=ora_nasterii
                    if field.find('varsta')>-1:
                        varsta = field.split('=')[1]
                        if varsta != "":
                            user['varsta']=varsta
                    if field.find('pagina_personala')>-1:
                        pagina_personala = field.split('=')[1]
                        if pagina_personala != "":
                            user['pagina_personala']=pagina_personala
                    if field.find('descriere')>-1:
                ...
                if(username!='' and parola!=''):
                    ...
                    mesaj=b"Parola si numele de utilizator nu trebuie sa fie nule"

                status = b"HTTP/1.1 200 OK\r\n"
        
        except Exception as e:
            print("EXCEPTIEEEEEE : "+str(e))
